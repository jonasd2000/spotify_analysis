...
class AnalysisHome(Page):
    ...
    def create_track_analysis_chart(self):
        ...
        hours_played = (most_listened_tracks['ms_played'] / 1000 / 3600).to_list()
        fig = {
            'data': [
                self._get_chart_data(x=track_names, y=hours_played, text=display_text),
            ],
    ...
    def create_artist_analysis_chart(self):
        ...
        hours_played = (most_listened_artists['ms_played'] / 1000 / 3600).to_list()
        
        fig = {
            'data': [
                self._get_chart_data(x=artist_names, y=hours_played, text=artist_names),
            ],
            'layout': self.top_graph_layouts,
            'config': self.top_graph_config,
        }
    ...
    def create_podcast_analysis_chart(self):
        ...
        hours_played = (most_listened_podcasts['ms_played'] / 1000 / 3600).to_list()
        
        fig = {
            'data': [
                self._get_chart_data(x=podcast_names, y=hours_played, text=podcast_names),
            ],
            'layout': self.top_graph_layouts,
            'config': self.top_graph_config,
        }
